...
data_review <- function(pt_data, dt_data, dm_data, no_cut){
  cyc_num <- 1
   sum_t <- tibble(Dataset = character(), 
                   ...
                   `Removed` = numeric(), 
                   `Modified` = numeric())
   if (is.null(pt_data) & is.null(dt_data) & is.null(dm_data) & is.null(no_cut)) {
...
df_tabs <- function(data){
  ...
  if (exists("DCUT_TEMP_DTHCHANGE", where = data)) {
    ...
    datatable(NULL)
      ...
      print(htmltools::tagList(DT::datatable(removed, filter = 'top', rownames = FALSE, 
    ...
    for (df in data){
      ...
      cat("####", toupper(name), " \n")
        ...
        print(htmltools::tagList(DT::datatable(removed, filter = 'top',
                                             options = list(scrollX = TRUE, fixedColumns = TRUE))))
      cat(' \n\n')
      cyc_num <- cyc_num + 1
    }}
...
sum_test <- function(data){
  ...
  if (is.null(data)) {
    ...
    cat("If you would like to apply this cut, please double check you have ran this cut / filled in all the parameters in process_cut() - for the wrapped approach - or read_out() - modular approach.")
    cat(' \n\n')
  } else {
