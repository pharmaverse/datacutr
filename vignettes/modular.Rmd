...
dataset_vignette(
  source_data$fa,
  display_vars = vars(USUBJID, FASTDTC, FADTC, DCUT_TEMP_FAXDTC)
)
```
...
dataset_vignette(
  patient_cut_data$sc,
  display_vars = vars(USUBJID, SCORRES, DCUT_TEMP_REMOVE)
)
```
...
dataset_vignette(
  date_cut_data$ae,
  display_vars = vars(USUBJID, AETERM, AESTDTC, DCUT_TEMP_SDTM_DATE, DCUT_TEMP_DCUTDTM, DCUT_TEMP_REMOVE)
)
```
...
dataset_vignette(
  dm_cut,
  display_vars = vars(USUBJID, DTHFL, DTHDTC, DCUT_TEMP_REMOVE, DCUT_TEMP_DTHDT, DCUT_TEMP_DCUTDTM, DCUT_TEMP_DTHCHANGE)
)
```
