...
As `{datacutr}` is intended to be contributed by the user community, this 
article is meant for developers that want to either expand `{datacutr}` functionalities or build on top of `{datacutr}`. 
...
* Each function has a specific purpose that supports the ADaM Dataset programming workflow. It could be an `{datacutr}` function or a company specific function.
* datacutr functions can be re-used for company specific functions.
...
wrap this into an datacutr function, as that would add an unnecessary extra layer for users.
...
(there are many utility functions in `{datacutr}` available to the contributor):
...
Inputs should be checked either using `asserthat::assert_that()` or custom assertion functions defined in [`R/assertions.R`](https://github.com/pharmaverse/datacutr/blob/main/R/assertions.R). 
...
This rule is applicable only if both functions are part of `{datacutr}`.
...
Functions from other packages have to be explicitly imported by using the `@importFrom` tag in the `R/datacutr-package.R` file. 
...
See [Writing Unit Tests in {datacutr}](unit_test_guidance.html#writing-unit-tests-in-datacutr-) for details.

# Deprecation

As `{datacutr}` is still evolving, functions or parameters may need to be removed or replaced with more 
...
* GitHub allows us through the Actions/Workflows to test `{datacutr}` under several versions of R as well as several versions of dependent R packages needed for `{datacutr}`.  Currently we test `{datacutr}` against 3.6.3 with a CRAN package snapshot from 2020-02-29, 4.0 with a CRAN package snapshot from 2021-03-31 and the latest R version with the latest snapshots of packages.  You can view this workflow on our [GitHub Repository](https://github.com/pharmaverse/datacutr/blob/main/.github/workflows/R-CMD-check.yml)
