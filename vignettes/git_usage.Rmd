...
This article will give you an overview of how the `{datacutr}` project is utilizing the version-control software `git` and the website GitHub while working with RStudio. We will go over the primary branches that house the source code for the `{datacutr}` project as well as how we use **Feature** branches to address **Issues**.  Issues can range from bugs to enhancement that have been identified or requested by developers, users or testers. Finally, we provide the bare minimum of `git` commands needed to get up and running. Please refer to the [Resource](#github_resources) section for more in-depth guidance on using `git` and GitHub. 

# Branches

-	The `main` branch contains the latest **released** version and should not be used for development. You can find the released versions [here](https://GitHub.com/pharmaverse/datacutr/releases) 
...
"Done" column on the "datacutr core board" by the developer. **The issue
