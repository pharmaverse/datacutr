...
dcut <- dcut %>% 
  ...
  mutate(DCUTDTC=as.Date(ifelse(!is.na(SVSTDTC),SVSTDTC,as.character(DCUTDTC)))) %>% 
...
Now that `DCUT` is prepared, the rest of the process follows the same as previously prescribed
